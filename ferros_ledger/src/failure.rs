...
//!
//! In the Ledger, failures are not exceptions to be caught and discarded.
//! They are typed VSF objects stored in the ledger itself — queryable,
//! inspectable, and part of the permanent record.
//!
//! A failed write doesn't disappear — it becomes a FailureRecord that
//! the mesh can reason about during arbitration.
//!
//! ## Contrast
//! - BTRFS: Errors surface as kernel log messages and errno returns.
//!   Failed writes in a transaction cause the entire transaction to
//!   abort — the failure itself is not recorded on disk. RAID scrub
//!   can detect and repair corruption, but the failure event is ephemeral.
//! - RedoxFS: Errors returned as `syscall::Error`. Failed writes cause
//!   the transaction to not commit (generation not incremented). No
//!   on-disk record of what failed or why.
//! - Ledger: FailureRecord is a VSF object with a hash, stored in the
//!   ledger. You can query "what failures occurred at generation N?"
//!   The mesh uses these records to make informed arbitration decisions.
...
pub struct FailureRecord {
    ...
    pub generation: u64,
    /// The object hash that was being written/read when the failure happened,
    /// if applicable.
    pub related_object: Option<ObjectHash>,
    /// The commit hash that was in progress when the failure happened,
    /// if applicable.
...
pub enum FailureKind {
    ...
    /// Power loss detected during write (device came back with
...
pub trait FailureLog {
    ...
    /// Record a new failure. Returns the hash of the failure record
    /// (it is itself a ledger object).
