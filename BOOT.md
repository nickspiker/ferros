...
1. **USB enumeration** (THE wall). Two things ruled out on hardware 2026-08-14, and the real work identified:
   - **CORRECTION** to the earlier "Qualcomm PHY" claim: the *active* `kernel_main` (feature `pixel8`) does NOT run the Qualcomm `phy_init()` (`QCOM_WRAPPER=G#0A6F8800`) — that's dead code only reached from the FP5 path at main.rs:2950. The live path runs an **inline eUSB PHY block** in `kernel_main` then a pure DWC3-core `Dwc3Dev::init()`. So Qualcomm code is not the bug.
   - **Experiment ruled out**: "trust ABL's PHY" (skip the inline PHY block + `warm_init` = DWC3-core-only, no PHY touch) → **no enumeration, 91s silent**. So ABL tears the eUSB2 PHY down on jump (like FP5); we must bring it up ourselves.
   - **Real bug**: the inline PHY block's register offsets (`EUSB_PHY+G#0/4/8/C`, `pll_fb_div=368`) do NOT match the real Exynos eUSB2 CR map (ref [tools/pixel8/phy-ref/phy-eusb.c](tools/pixel8/phy-ref/phy-eusb.c): ref-freq@G#18, termselect@G#8, opmode@G#C, PLL timing@G#200-20C, PLL-lock poll@G#220), and it's missing the mandatory pre-steps: **eUSB repeater power-on (I2C), PMU isolation clear, BLKCON link init**. The core CR-register sequence lives in `phy-exynos-eusb.c` — **which we don't have** (only the framework layer phy-eusb.c + the reg header are in phy-ref/).
   - **Two unblocks needed**: (a) the `phy-exynos-eusb.c` core-init source (or reverse-engineer ~14 CR writes from the header — slow, error-prone blind); (b) an **output channel** to iterate. The kernel already writes a UFS diagnostic (`"FERROS v3 S2MPU_BYPASS=OK"` + SNPSID/GCTL/DSTS/USB2PHY) to the ferros partition LBA — investigate `fastboot fetch` (Pixel unlocked-mode partition read) or a USB-C SBU serial console (Suzy-Q) to read state back.
