...
3. **RELOAD hot-reload**: dispatch arms exist (`Write` counts bytes, `Exec` acks); staging to high DRAM + the verified jump are TODO. With PT transport now solid this is the next lever — kills the fastboot flash cycle entirely.
