//! `Store<D>` — the device-generic vault [`crate::store::ObjectStore`] implementation.
//!
//! Owns a `D: Device`, an [`crate::anchor::AnchorKey`], the latest [`VaultAnchor`], and an in-memory hash → offset index of all objects appended so far. Append-only writes (new objects go past `object_tail`); reads consult the index for O(1) lookup; the anchor ring records the current `root_commit + object_tail` after each write.
//!
//! This is the no_std core of the vault backend: it talks to storage exclusively through the [`crate::device::Device`] trait, so the exact same logic drives the host-file proving ground ([`crate::host_file::FileStore`] = `Store<FileDevice>`) and the on-hardware UFS-backed store (`Store<UfsDevice>`). Nothing here is device-specific.
//!
...
use crate::vault_anchor::{self, VaultAnchor, SLOT_STRIDE, SLOT_ZERO_OFFSET};
...
/// Store — the ObjectStore impl backed by any [`Device`] `D`.
...
/// Errors specific to backend Store operations beyond the trait's [`StoreError`].
#[derive(Debug)]
pub enum StoreBackendError {
    Device(DeviceError),
    Anchor(vault_anchor::AnchorError),
    Wrapper(crate::vsf_wrapper::WrapperError),
...
impl<D: Device> Store<D> {
    ...
    pub fn open(device: D, anchor_key: AnchorKey) -> Result<Self, StoreBackendError> {
        ...
        if payload.len() < (SLOT_ZERO_OFFSET as usize) + SLOT_STRIDE as usize {
            return Err(StoreBackendError::PayloadTooSmall {
        ...
        let slot0_anchor =
            vault_anchor::decode(slot0_bytes, &anchor_key).map_err(StoreBackendError::Anchor)?;
    ...
    pub fn format(
        ...
        let wrapped = crate::vsf_wrapper::encode(&payload).map_err(StoreBackendError::Wrapper)?;
        if wrapped.len() as u64 > device.capacity() {
            // Grow the file to fit.
            device
                .set_capacity(wrapped.len() as u64)
                .map_err(StoreBackendError::Device)?;
        }
    ...
    pub fn load_root_commit(&self) -> Result<crate::root_commit::RootCommit, StoreError> {
        let obj = self.get(&self.anchor.root_commit)?;
        crate::root_commit::RootCommit::decode(&obj.content).map_err(|_| StoreError::IntegrityViolation {
    ...
    fn write_anchor(&mut self, anchor: &VaultAnchor) -> Result<(), StoreError> {
        ...
        let wrapped = crate::vsf_wrapper::encode(&payload).map_err(|_| {
