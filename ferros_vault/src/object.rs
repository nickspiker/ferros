...
//!
//! Every piece of data in the Ledger is a VSF object: immutable, content-
//! addressed, and self-describing via elastic-width encoding (EWE).
//!
//! ## Contrast
//! - BTRFS: Items are typed by a u8 in the btrfs_key (INODE_ITEM=1,
//!   DIR_ITEM=12, EXTENT_DATA=108, etc.). Fixed 17-byte keys. Item data
//!   lives in leaf nodes with fixed 4K-64K node sizes.
...
//!   exactly as wide as its value requires. A 1-byte object takes 1 byte
//!   plus its (tiny) header. No padding, no alignment waste.
...
///
/// These correspond to VSF primitive and structured types.
/// `g`, `k`, `h`, `a` are the VSF crypto primitives used at Layer 1.
...
///
/// This is what gets stored alongside the content in the object store.
/// The hash is derived from content + metadata fields, not stored separately.
#[derive(Clone, Debug)]
pub struct ObjectMeta {
    ...
    pub domain: Vec<u8>,
    /// Byte length of the content. Redundant (derivable from content)
    /// but useful for allocation without reading content.
    pub content_len: u64,
...
///
/// Immutable after creation. To "modify" an object, create a new one
/// with updated content — it will have a different hash (different identity).
#[derive(Clone, Debug)]
pub struct Object {
    pub meta: ObjectMeta,
...
///
/// This is how higher-level structures (capability tokens, mesh records,
/// failure states) get stored in the ledger — they serialize to Object.
